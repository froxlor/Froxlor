...
if ($page === 'overview') {
    ...
    if ($action === '') {
        ...
        $result_stmt = Database::prepare(
            'SELECT `id`, `username`, `description`, `homedir`, `shell` FROM `' . TABLE_FTP_USERS . '`
        ...
        $result_stmt = Database::prepare(
            'SELECT `id`, `username`, `homedir`, `up_count`, `up_bytes`, `down_count`, `down_bytes` FROM `' . TABLE_FTP_USERS . '`
        ...
        if (isset($result['username']) && $result['username'] !== $userinfo['loginname']) {
            if (isset($_POST['send']) && $_POST['send'] === 'send') {
                ...
                $result_stmt = Database::prepare(
                    'SELECT `username`, `homedir` FROM `' . TABLE_FTP_USERS . '`
					WHERE `customerid` = :customerid
					AND `id` = :id'
                );
                Database::pexecute($result_stmt, array('customerid' => $userinfo['customerid'], 'id' => $id));
        ...
        if ($userinfo['ftps_used'] < $userinfo['ftps'] || $userinfo['ftps'] === '-1') {
            if (isset($_POST['send'])
                ...
                if (Settings::Get('customer.ftpatdomain') === '1') {
                    ...
                    $ftpdomain_check_stmt = Database::prepare(
                ...
                if (!empty($username_check) && $username_check['username'] = $username) {
                    ...
                    $result_stmt = Database::prepare(
                        'SELECT `bytes_in_used` FROM `' . TABLE_FTP_QUOTATALLIES . '`
						WHERE `name` = :name'
                    );
                    ...
                    while ($row = $result_stmt->fetch(PDO::FETCH_ASSOC)) {
                        $stmt = Database::prepare(
                            'INSERT INTO `' . TABLE_FTP_QUOTATALLIES . "`
                ...
                $ftp_add_data = include_once __DIR__ . '/lib/formfields/customer/ftp/formfield.ftp_add.php';
        ...
        $result_stmt = Database::prepare(
            'SELECT `id`, `username`, `description`, `homedir`, `uid`, `gid`, `shell` FROM `' . TABLE_FTP_USERS . '`
			WHERE `customerid` = :customerid
			AND `id` = :id'
        );
        Database::pexecute($result_stmt, array('customerid' => $userinfo['customerid'], 'id' => $id));
        ...
        if (isset($result['username']) && $result['username'] !== '') {
            if (isset($_POST['send']) && $_POST['send'] === 'send') {
                ...
                $ftp_edit_data = include_once __DIR__ . '/lib/formfields/customer/ftp/formfield.ftp_edit.php';
