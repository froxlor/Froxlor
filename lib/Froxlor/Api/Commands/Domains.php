...
class Domains extends \Froxlor\Api\ApiCommand implements \Froxlor\Api\ResourceEntity
{
	 ...
	 * @param bool $nginx_root_default
	 *        	optional, enable default root directive for nginx, default 1 (true)
	 * @param bool $nginx_location_default
	 *        	optional, enable default location / {} server block for nginx, default 1 (true)
	...
	public function add()
	{
		if ($this->isAdmin()) {
			if ($this->getUserDetail('domains_used') < $this->getUserDetail('domains') || $this->getUserDetail('domains') == '-1') {
				...
				$nginx_root_default = $this->getBoolParam('nginx_root_default', true, 1);
				$nginx_location_default = $this->getBoolParam('nginx_location_default', true, 1);
				...
				if ($this->getUserDetail('change_serversettings') == '1') {
					...
					$writeaccesslog = '1';
					$writeerrorlog = '1';
					$nginx_root_default = '1';
					$nginx_location_default = '1';
					$override_tls = '0';
					$ssl_protocols = array();
				}
				...
				if ($domain == '') {
					...
					$ins_data = array(
						...
						'nginx_root_default' => $nginx_root_default,
						'nginx_location_default' => $nginx_location_default,
	...
	public function update()
	{
		if ($this->isAdmin()) {
			...
			$nginx_root_default = $this->getBoolParam('nginx_root_default', true, $result['nginx_root_default']);
			$nginx_location_default = $this->getBoolParam('nginx_location_default', true, $result['nginx_location_default']);
			...
			if ($this->getUserDetail('change_serversettings') == '1') {
				...
				$nginx_root_default = $result['nginx_root_default'];
				$nginx_location_default = $result['nginx_location_default'];
			...
			if ($documentroot != $result['documentroot'] || $ssl_redirect != $result['ssl_redirect'] || $wwwserveralias != $result['wwwserveralias'] || $iswildcarddomain != $result['iswildcarddomain'] || $phpenabled != $result['phpenabled'] || $openbasedir != $result['openbasedir'] || $phpsettingid != $result['phpsettingid'] || $mod_fcgid_starter != $result['mod_fcgid_starter'] || $mod_fcgid_maxrequests != $result['mod_fcgid_maxrequests'] || $specialsettings != $result['specialsettings'] || $ssl_specialsettings != $result['ssl_specialsettings'] || $notryfiles != $result['notryfiles'] || $writeaccesslog != $result['writeaccesslog'] || $writeerrorlog != $result['writeerrorlog'] || $nginx_root_default != $result['nginx_root_default'] || $nginx_location_default != $result['nginx_location_default'] || $aliasdomain != $result['aliasdomain'] || $issubof != $result['ismainbutsubto'] || $email_only != $result['email_only'] || ($speciallogfile != $result['speciallogfile'] && $speciallogverified == '1') || $letsencrypt != $result['letsencrypt'] || $http2 != $result['http2'] || $hsts_maxage != $result['hsts'] || $hsts_sub != $result['hsts_sub'] || $hsts_preload != $result['hsts_preload'] || $ocsp_stapling != $result['ocsp_stapling']) {
				\Froxlor\System\Cronjob::inserttask(\Froxlor\Cron\TaskId::REBUILD_VHOST);
			}
			...
			$update_data['nginx_root_default'] = $nginx_root_default;
			$update_data['nginx_location_default'] = $nginx_location_default;
			...
			$_update_data['nginx_root_default'] = $nginx_root_default;
			$_update_data['nginx_location_default'] = $nginx_location_default;
