...
class Rspamd extends DkimCron
{
	...
	protected function updateConfig() 
	{
		...
		if (!empty($rspamd_selector_map_setting)) {
			...
			if (file_put_contents($rspamd_selector_map_filename, $this->rspamd_dkim_selectors_map) === false) {
				$this->logger->logAction(\Froxlor\FroxlorLogger::CRON_ACTION, LOG_ERR, "Write dkim.rspamd_dkim_selector_map failed");
			}
			\Froxlor\FileDir::safe_exec("chmod 0644 " . escapeshellarg($rspamd_selector_map_filename));
		}
		...
		if (!empty($rspamd_paths_map_setting)) {
			...
			if (file_put_contents($rspamd_paths_map_filename, $this->rspamd_dkim_paths_map) === false) {
				$this->logger->logAction(\Froxlor\FroxlorLogger::CRON_ACTION, LOG_ERR, "Write dkim.rspamd_dkim_paths_map failed");
			}
