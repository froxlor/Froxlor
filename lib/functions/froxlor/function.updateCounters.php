...
function updateCounters($returndebuginfo = false)
{
    ...
    while ($customer = $customers_stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        $customer_mysqls_stmt = Database::prepare(
            'SELECT COUNT(*) AS `number_mysqls` FROM `' . TABLE_PANEL_DATABASES . '`
        ...
        $customer_emails_stmt = Database::prepare(
        
            'SELECT COUNT(*) AS `number_emails` FROM `' . TABLE_MAIL_VIRTUAL . '`
			WHERE `customerid` = :cid'
        );
        ...
        $customer_emails_result_stmt = Database::prepare(
        
            'SELECT `email`, `email_full`, `destination`, `popaccountid` AS `number_email_forwarders` FROM `' . TABLE_MAIL_VIRTUAL . '`
			WHERE `customerid` = :cid'
        );
        ...
        while ($customer_emails_row = $customer_emails_result_stmt->fetch(PDO::FETCH_ASSOC)) {
            if ($customer_emails_row['destination'] !== '') {
                ...
                if (in_array($customer_emails_row['email_full'], $customer_emails_row['destination'], true)) {
