...
class ConfigDaemon
{
    ...
    private function _checkVisibility($order)
    {
        ...
        switch ($attributes['mode']) {
            case 'isfile': if (!is_file($order)) {
                $return = -1;
            }; break;
            case 'notisfile': if (is_file($order)) {
                $return = -1;
            }; break;
            case 'isdir': if (!is_dir($order)) {
                $return = -1;
            }; break;
            case 'notisdir': if (is_dir($order)) {
                $return = -1;
            }; break;
            case 'false': if ($order === true) {
                $return = -1;
            }; break;
            case 'true': if ($order === false) {
                $return = -1;
            }; break;
            case 'notempty': if ($order === '') {
                $return = -1;
            }; break;
            case 'userexists': if (posix_getpwuid($order) === false) {
                $return = -1;
            }; break;
            case 'groupexists': if (posix_getgrgid($order) === false) {
                $return = -1;
            }; break;
            case 'usernotexists': if (is_array(posix_getpwuid($order))) {
                $return = -1;
            }; break;
            case 'groupnotexists': if (is_array(posix_getgrgid($order))) {
                $return = -1;
            }; break;
            case 'usernamenotexists': if (is_array(posix_getpwnam($order))) {
                $return = -1;
            }; break;
