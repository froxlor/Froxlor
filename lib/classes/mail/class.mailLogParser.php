...
class MailLogParser
{
            ...
            if ($this->startTime < $timestamp) {
                if (preg_match("/postfix\/qmgr.*(?::|\])\s([A-Z\d]+).*from=<?(?:.*\@([a-z\A-Z\d\.\-]+))?>?, size=(\d+),/", $line, $matches)) {
                    ...
                    if (array_key_exists($matches[1], $this->mails)) {
                        ...
                        if (in_array($mail['domainFrom'], $this->myDomains, true) || in_array($mail['domainTo'], $this->myDomains, true)) {
                            // Outgoing traffic
                            ...
                            // Incoming traffic
                            if (array_key_exists('domainTo', $mail) && in_array($mail['domainTo'], $this->myDomains, true)) {
    ...
    private function _addDomainTraffic($domain, $traffic, $timestamp)
    {
        ...
        if (in_array($domain, $this->myDomains, true)) {
