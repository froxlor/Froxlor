...
if ($page === 'overview') {
    ...
    if ($action === '') {
        ...
        $result_stmt = Database::prepare(
            'SELECT `m`.`id`, `m`.`domainid`, `m`.`email`, `m`.`email_full`, `m`.`iscatchall`, `u`.`quota`, `m`.`destination`, `m`.`popaccountid`, `d`.`domain`, `u`.`mboxsize` FROM `' . TABLE_MAIL_VIRTUAL . '` `m`
        ...
        $emaildomains_count_stmt = Database::prepare(
        ...
        $stmt = Database::prepare(
            'SELECT `id`, `email`, `email_full`, `iscatchall`, `destination`, `customerid`, `popaccountid` FROM `' . TABLE_MAIL_VIRTUAL . '`
			WHERE `customerid`= :customerid
			AND `id`= :id'
        );
                ...
                if ($result['destination'] !== '') {
                    ...
                    if ($result['popaccountid'] !== 0) {
                        ...
                        if (Settings::Get('system.mail_quota_enabled') === 1) {
                            $stmt = Database::prepare(
                                'SELECT `quota` FROM `' . TABLE_MAIL_USERS . '`
								WHERE `customerid`= :customerid
								AND `id`= :id'
                            );
        ...
        if ($userinfo['emails_used'] < $userinfo['emails'] || $userinfo['emails'] === '-1') {
            if (isset($_POST['send']) && $_POST['send'] === 'send') {
                ...
                $stmt = Database::prepare(
                    'SELECT `id`, `domain`, `customerid` FROM `' . TABLE_PANEL_DOMAINS . "`
					WHERE `domain`= :domain
                ...
                if ($email === '' || $email_full === '' || $email_part === '') {
                    ...
                    $stmt = Database::prepare(
                        'INSERT INTO `' . TABLE_MAIL_VIRTUAL . '`
                ...
                $email_add_data = include_once __DIR__ . '/lib/formfields/customer/email/formfield.emails_add.php';
        ...
        $stmt = Database::prepare(
            'SELECT `v`.`id`, `v`.`email`, `v`.`email_full`, `v`.`iscatchall`, `v`.`destination`, `v`.`customerid`, `v`.`popaccountid`, `u`.`quota`
        ...
        if (isset($result['email']) && $result['email'] !== '') {
            ...
            $email_edit_data = include_once __DIR__ . '/lib/formfields/customer/email/formfield.emails_edit.php';
    ...
    if ($action === 'add' && $id !== 0) {
        ...
        if ($userinfo['email_accounts'] === '-1' || ($userinfo['email_accounts_used'] < $userinfo['email_accounts'])) {
            ...
            if (isset($result['email']) && $result['email'] !== '' && $result['popaccountid'] === '0') {
                if (isset($_POST['send']) && $_POST['send'] === 'send') {
                    ...
                    $account_add_data = include_once __DIR__ . '/lib/formfields/customer/email/formfield.emails_addaccount.php';
        ...
        if (isset($result['popaccountid']) && $result['popaccountid'] !== '') {
            if (isset($_POST['send']) && $_POST['send'] === 'send') {
                ...
                $account_changepw_data = include_once __DIR__ . '/lib/formfields/customer/email/formfield.emails_accountchangepasswd.php';
        ...
        if (isset($result['popaccountid']) && $result['popaccountid'] !== '') {
            if (isset($_POST['send']) && $_POST['send'] === 'send') {
                ...
                $quota_edit_data = include_once __DIR__ . '/lib/formfields/customer/email/formfield.emails_accountchangequota.php';
    ...
    if ($action === 'add' && $id !== 0) {
        if ($userinfo['email_forwarders_used'] < $userinfo['email_forwarders'] || $userinfo['email_forwarders'] === '-1') {
            $stmt = Database::prepare(
                'SELECT `id`, `email`, `email_full`, `iscatchall`, `destination`, `customerid`, `popaccountid`, `domainid` FROM `' . TABLE_MAIL_VIRTUAL . '`
				WHERE `customerid`= :cid
				AND `id`= :id'
            );
            $result = Database::pexecute_first($stmt, array('cid' => $userinfo['customerid'], 'id' => $id));
            ...
            if (isset($result['email']) && $result['email'] !== '') {
                if (isset($_POST['send']) && $_POST['send'] === 'send') {
                    ...
                    } elseif (in_array($destination, $result['destination_array'], true)) {
                    ...
                    $forwarder_add_data = include_once __DIR__ . '/lib/formfields/customer/email/formfield.emails_addforwarder.php';
