...
if ($page === 'overview') {
    ...
    if ($action === '') {
        ...
        $domains_stmt = Database::prepare(
            'SELECT `d`.`id`, `d`.`customerid`, `d`.`domain`, `d`.`documentroot`, `d`.`isbinddomain`, `d`.`isemaildomain`, `d`.`caneditdomain`, `d`.`iswildcarddomain`, `d`.`parentdomainid`, `d`.`letsencrypt`, `d`.`termination_date`, `ad`.`id` AS `aliasdomainid`, `ad`.`domain` AS `aliasdomain`, `da`.`id` AS `domainaliasid`, `da`.`domain` AS `domainalias` FROM `' . TABLE_PANEL_DOMAINS . '` `d`
        ...
        if (isset($result['parentdomainid']) && $result['parentdomainid'] !== '0' && $alias_check['count'] === 0) {
            if (isset($_POST['send']) && $_POST['send'] === 'send') {
                ...
                $stmt = Database::prepare(
                    'DELETE FROM `' . TABLE_PANEL_DOMAINS . '` WHERE
            ...
            if (isset($_POST['send']) && $_POST['send'] === 'send') {
                ...
                if ($path === '') {
                    ...
                    $phpsid_stmt = Database::prepare(
                        'SELECT `phpsettingid` FROM `' . TABLE_PANEL_DOMAINS . '`
						WHERE `id` = :id'
                    );
                ...
                $subdomain_add_data = include_once __DIR__ . '/lib/formfields/customer/domains/formfield.domains_add.php';
        ...
        if (isset($result['customerid']) && $result['customerid'] === $userinfo['customerid']) {
            if (isset($_POST['send']) && $_POST['send'] === 'send') {
                ...
                // also check ip/port combination to be the same, #176
                $domains_stmt = Database::prepare(
                    'SELECT `d`.`id`, `d`.`domain` FROM `' . TABLE_PANEL_DOMAINS . '` `d` , `' . TABLE_PANEL_CUSTOMERS . '` `c` , `' . TABLE_DOMAINTOIP . "` `dip`
					WHERE `d`.`aliasdomain` IS NULL
					AND `d`.`id` <> :id
					AND `c`.`standardsubdomain` <> `d`.`id`
					AND `d`.`parentdomainid` = '0'
                ...
                $ips_stmt = Database::prepare(
                    'SELECT `p`.`ip` AS `ip` FROM `' . TABLE_PANEL_IPSANDPORTS . '` `p`
                ...
                $subdomain_edit_data = include_once __DIR__ . '/lib/formfields/customer/domains/formfield.domains_edit.php';
    ...
    if ($action === '' || $action === 'view') {
        if (isset($_POST['send']) && $_POST['send'] === 'send') {
            ...
            $stmt = Database::prepare(
                $qrystart . ' `' . TABLE_PANEL_DOMAIN_SSL_SETTINGS . '` SET
        ...
        $ssleditor_data = include_once __DIR__ . '/lib/formfields/customer/domains/formfield.domain_ssleditor.php';
