...
class Action
{
    ...
    private function _createConfig()
    {
        ...
        while (! in_array($_daemons_config['distro'], $distributions_select_data, true)) {
        ...
        if (! empty($decoded_config)) {
            ...
            foreach ($services as $si => $service) {
                ...
                foreach ($daemons as $di => $dd) {
                    ...
                    if (($si !== 'system' && $decoded_config[$si] !== $di) || (is_array($decoded_config[$si]) && ! in_array($di, $decoded_config[$si], true))) {
                        continue;
                    }
