...
{
	showUpdateStep("Updating from 0.9.28-svn5 to 0.9.28-svn6", false);
	...
	while($row = $db->fetch_array($result))
	{
		...
		elseif ((int)$row['ssl_ipandport'] == 0 && (int)$row['ssl_redirect'] != 0 && (int)$row['parentdomainid'] != 0) {	// Subdomain, erhält auch SSL Port!
			$db->query("INSERT INTO `".TABLE_DOMAINTOIP."` SET
				`id_domain` = " . (int)$row['id'] . ",
