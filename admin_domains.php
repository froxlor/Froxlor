    ...
    if ($action === '') {
        ...
        if ($userinfo['domains_used'] < $userinfo['domains'] || $userinfo['domains'] === '-1') {
            if (isset($_POST['send']) && $_POST['send'] === 'send') {
                ...
                $domain_add_data = include_once __DIR__ . '/lib/formfields/admin/domains/formfield.domains_add.php';
        ...
        if ($result['domain'] !== '') {
            ...
            while ($domain_emails_row = $domain_emails_result_stmt->fetch(PDO::FETCH_ASSOC)) {
                if ($domain_emails_row['destination'] !== '') {
                    ...
                    if (in_array($domain_emails_row['email_full'], $domain_emails_row['destination'], true)) {
                        $email_forwarders -= 1;
                        $email_accounts ++;
                    }
                }
            ...
            if (isset($_POST['send']) && $_POST['send'] === 'send') {
                ...
                while ($phpconfigs_row = $phpconfigs_result_stmt->fetch(PDO::FETCH_ASSOC)) {
                    $disabled = !empty($c_allowed_configs) && !in_array($phpconfigs_row['id'], $c_allowed_configs, true);
                    if ((int) Settings::Get('phpfpm.enabled') === 1) {
                ...
                $domain_edit_data = include_once __DIR__ . '/lib/formfields/admin/domains/formfield.domains_edit.php';
        ...
        if (isset($_POST['send']) && $_POST['send'] === 'send') {
            ...
            $domain_import_data = include_once __DIR__ . '/lib/formfields/admin/domains/formfield.domains_import.php';
