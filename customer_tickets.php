...
if ($page === 'overview') {
    ...
    if ($action === '') {
        ...
        $stmt = Database::prepare(
            'SELECT `main`.`id`, (SELECT COUNT(`sub`.`id`) FROM `' . TABLE_PANEL_TICKETS . '` `sub`
        ...
        if ($userinfo['tickets_used'] < $userinfo['tickets'] || $userinfo['tickets'] === '-1') {
            if (isset($_POST['send']) && $_POST['send'] === 'send') {
                ...
                $ticket_add_data = include_once __DIR__ . '/lib/formfields/customer/tickets/formfield.ticket_add.php';
        ...
        if (isset($_POST['send']) && $_POST['send'] === 'send') {
            ...
            $result_stmt = Database::prepare(
                'SELECT `name` FROM `' . TABLE_PANEL_TICKET_CATS . '`
				WHERE `id`= :id '
            );
            ...
            $ticket_reply_data = include_once __DIR__ . '/lib/formfields/customer/tickets/formfield.ticket_reply.php';
