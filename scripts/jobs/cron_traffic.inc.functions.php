...
function awstatsDoSingleDomain($domain, $outputdir)
{
	...
	if(file_exists($domainconfig))
	{
        ...
        $outputdir_stat = makeCorrectDir($outputdir . '/' . $year . '/' . $month);
		if(!is_dir($outputdir))
		{
			safe_exec('mkdir -p ' . escapeshellarg($outputdir));
		}
		
        if(!is_dir($outputdir_stat))
		{
			safe_exec('mkdir -p ' . escapeshellarg($outputdir_stat));
		}
		...
		safe_exec($awbsp.' -awstatsprog='.escapeshellarg($awprog).' -update -lang=DE -month=' . date('n') . ' -year=' . date('Y') . ' -config=' . $domain . ' -dir='.escapeshellarg($outputdir_stat));
		...
		$model_file = dirname(dirname(dirname(__FILE__)));
    	$model_file.= '/templates/misc/awstatsmodel/';
        $model_file.= 'awstats.froxlor.index.php';   	
    
    	$model_file = makeCorrectFile($model_file);

    	$new_index = makeCorrectFile($outputdir.'/index.php');
	if (is_file(makeCorrectFile($outputdir.'/index.html')))
		safe_exec('rm ' . makeCorrectFile($outputdir.'/index.html'));
		
        $awstats_index = fopen($new_index, 'w');
    	$awstats_model_index = fopen($model_file, 'r');
    
    	// Write the header
    
    	// Write the configuration file
    
    	while(($line = fgets($awstats_model_index, 4096)) !== false)
    	{
    		if(!preg_match('/^#/', $line)
    		   && trim($line) != '')
    		{
    			fwrite($awstats_index, preg_replace($regex, $replace, $line));
    		}
    	}
    
    	fclose($awstats_index);
    	fclose($awstats_model_index);
    	
    	$original_index = makeCorrectFile($outputdir_stat.'/awstats.'.$domain.'.html');
		$new_index = makeCorrectFile($outputdir_stat.'/index.html');
